...
class TransporterAgent:
    ...
    def get_sample_place(self, dataset, augment=True):
        ...
        p0_place = utils.xyz_to_pix(p0_xyz_place, self.bounds_place, self.pix_size)
        p0_theta_place = -np.float32(utils.quatXYZW_to_eulerXYZ(p0_xyzw_place)[2])
        p1_place = utils.xyz_to_pix(p1_xyz_place, self.bounds_place, self.pix_size)
        p1_theta_place = -np.float32(utils.quatXYZW_to_eulerXYZ(p1_xyzw_place)[2])
        p1_theta_place = p1_theta_place - p0_theta_place
        p0_theta_place = 0

        # Data augmentation.
        if augment:
            img_place, _, (p0_place, p1_place), _ = utils.perturb(img_place, [p0_place, p1_place])
    ...
    def train(self, dataset, writer=None):
        ...
        img_place, p0_place, p0_theta_place, p1_place, p1_theta_place = self.get_sample_place(dataset)
        ...
        if isinstance(self.transport, Attention):
            loss1 = self.transport.train(img_place, p1_place, p1_theta_place)
        else:
            loss1 = self.transport.train(img_place, p0, p1_place, p1_theta_place)
    ...
    def act(self, obs, info=None, goal=None):  # pylint: disable=unused-argument
        ...
        place_conf = self.transport.forward(img_place, p0_pix)
        ...
        p1_xyz = utils.pix_to_xyz(p1_pix, hmap_place, self.bounds_place, self.pix_size)
