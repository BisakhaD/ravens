...
def get_sideways_heightmap(points, colors, bounds, pixel_size):
    """Get sideways (y-axis) orthographic heightmap image from 3D pointcloud.
  ...
  """
    width = int(np.round((bounds[0, 1] - bounds[0, 0]) / pixel_size))
    length = int(np.round((bounds[2, 1] - bounds[2, 0]) / pixel_size))
    heightmap = np.zeros((length, width), dtype=np.float32)
    colormap = np.zeros((length, width, colors.shape[-1]), dtype=np.uint8)

    # print(bounds, bounds[0, 1], bounds[0, 0], bounds[0, 1] - bounds[0, 0], "\n \n \n")
    ...
    # Sort 3D points by y-value, which works with array assignment to simulate
    # y-buffering for rendering the heightmap image.
    iy = np.argsort(points[:, -1])
    points, colors = points[iy], colors[iy]
    px = np.int32(np.floor((points[:, 0] - bounds[0, 0]) / pixel_size))
    pz = np.int32(np.floor((points[:, 2] - bounds[2, 0]) / pixel_size))
    px = np.clip(px, 0, width - 1)
    pz = np.clip(pz, 0, length - 1)
    heightmap[pz, px] = points[:, 1] - bounds[1, 0]
    for c in range(colors.shape[-1]):
        colormap[pz, px, c] = colors[:, c]
    return heightmap, colormap
...
def get_pointcloud(depth, intrinsics):
  ...
  """
    ...
    # next lines were to visualise but it needs to be n x 3, current shape is h x w x 3
...
def reconstruct_sideways_heightmaps(color, depth, configs, bounds, pixel_size):
    """Reconstruct sideways heightmap views from multiple 3D pointclouds."""
    heightmaps, colormaps = [], []
    for color, depth, config in zip(color, depth, configs):
        intrinsics = np.array(config['intrinsics']).reshape(3, 3)
        xyz = get_pointcloud(depth, intrinsics)
        ...
        transform[:3, :] = np.hstack((rotation, position))
        xyz = transform_pointcloud(xyz, transform)
        heightmap, colormap = get_sideways_heightmap(xyz, color, bounds, pixel_size)
        heightmaps.append(heightmap)
        colormaps.append(colormap)
    return heightmaps, colormaps
...
def get_sideways_fused_heightmap(obs, configs, bounds, pix_size):
    """Reconstruct orthographic heightmaps with segmentation masks."""
    heightmaps, colormaps = reconstruct_sideways_heightmaps(
        obs['color'], obs['depth'], configs, bounds, pix_size)
    colormaps = np.float32(colormaps)
    heightmaps = np.float32(heightmaps)
...
def plot(fname,  # pylint: disable=dangerous-default-value
    ...
    for name, (x, y, std) in data.items():
        ...
        plt.plot(xs, ys, color=color, linewidth=2, marker=',', alpha=1.)
        color_iter += 1
